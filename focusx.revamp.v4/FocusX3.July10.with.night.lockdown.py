...
class PomodoroBlocker:
    def __init__(self):
        ...
        self.sleep_overlay = None # Reference to the fullscreen overlay for sleep time
        ...
        self.sleep_start_minute = None
        self.sleep_monitoring_thread = None

        # Synchronize time on startup
        self.sync_time()
        
        # Ask for sleep time after UI is set up and time is synced
        self.root.after(100, self.ask_for_sleep_time) # Delay to ensure UI is ready
    ...
    def ask_for_sleep_time(self):
        """
        Prompts the user to set their desired sleep time.
        This is like setting an alarm for your bedtime. It ensures the app
        knows when to start encouraging you to wind down.
        """
        # Create a Toplevel window for the sleep time input
        sleep_dialog = tk.Toplevel(self.root)
        sleep_dialog.title("Set Sleep Time")
        sleep_dialog.transient(self.root) # Make it appear on top of the main window
        sleep_dialog.grab_set() # Make it modal (block interaction with main window)
        ...
        sleep_dialog.geometry(f'{dialog_width}x{dialog_height}+{center_x}+{center_y}')
        sleep_dialog.resizable(False, False)
        sleep_dialog.configure(bg=self.colors['bg'])

        tk.Label(sleep_dialog, text="Set your sleep time (HH:MM PM/AM):", bg=self.colors['bg'], fg=self.colors['text'], font=('Helvetica', 12)).pack(pady=10)

        time_frame = tk.Frame(sleep_dialog, bg=self.colors['bg'])
        time_frame.pack(pady=5)

        self.sleep_hour_var = tk.StringVar(value="09") # Default to 9 PM
        self.sleep_minute_var = tk.StringVar(value="00")
        self.sleep_ampm_var = tk.StringVar(value="PM")

        # Hour Spinbox
        hour_spinbox = ttk.Spinbox(time_frame, from_=1, to=12, textvariable=self.sleep_hour_var, width=3, format="%02.0f")
        hour_spinbox.pack(side=tk.LEFT, padx=2)
        tk.Label(time_frame, text=":", bg=self.colors['bg'], fg=self.colors['text']).pack(side=tk.LEFT)
        # Minute Spinbox
        minute_spinbox = ttk.Spinbox(time_frame, from_=0, to=59, textvariable=self.sleep_minute_var, width=3, format="%02.0f")
        minute_spinbox.pack(side=tk.LEFT, padx=2)
        # AM/PM OptionMenu
        ampm_menu = ttk.OptionMenu(time_frame, self.sleep_ampm_var, "PM", "AM", "PM")
        ampm_menu.pack(side=tk.LEFT, padx=5)

        def set_sleep_time_callback():
            try:
                hour = int(self.sleep_hour_var.get())
                minute = int(self.sleep_minute_var.get())
                ampm = self.sleep_ampm_var.get()

                if ampm == "PM" and hour != 12:
                    hour += 12
                elif ampm == "AM" and hour == 12: # 12 AM is 00 hour
                    hour = 0
                ...
                self.sleep_start_hour = hour
                self.sleep_start_minute = minute
                sleep_dialog.destroy()
                self.start_sleep_monitoring() # Start monitoring once time is set
            except ValueError:
                messagebox.showerror("Invalid Input", "Please enter valid numbers for hour and minute.")

        ok_button = tk.Button(sleep_dialog, text="Set Time", command=set_sleep_time_callback, bg=self.colors['primary'], fg='white', font=('Helvetica', 10))
        ok_button.pack(pady=10)

        # Wait for the dialog to close
        self.root.wait_window(sleep_dialog)

    def start_sleep_monitoring(self):
        """
        Starts a background thread to monitor for the set sleep time.
        This is like a diligent night watchman, constantly checking the clock
        to see if it's past your designated bedtime.
        """
        if self.sleep_start_hour is None or self.sleep_start_minute is None:
            print("Sleep time not set, sleep monitoring not started.")
            return

        if self.sleep_monitoring_thread and self.sleep_monitoring_thread.is_alive():
            # If already running, stop the old one first
            self.sleep_monitoring_thread_running = False
            self.sleep_monitoring_thread.join()

        self.sleep_monitoring_thread_running = True
        self.sleep_monitoring_thread = threading.Thread(target=self._monitor_sleep_time, daemon=True)
        self.sleep_monitoring_thread.start()
        print(f"Sleep monitoring started for {self.sleep_start_hour:02d}:{self.sleep_start_minute:02d}")

    def _monitor_sleep_time(self):
        """
        The actual loop that checks the current time against the sleep time.
        This is the clock's internal mechanism, ticking away and comparing
        the current moment to your set sleep schedule.
        """
        while self.sleep_monitoring_thread_running:
            current_time = self.get_accurate_time()
            
            # Define the sleep start time for today
            sleep_start_today = current_time.replace(hour=self.sleep_start_hour, minute=self.sleep_start_minute, second=0, microsecond=0)
            
            # Define midnight for today
            midnight_today = current_time.replace(hour=0, minute=0, second=0, microsecond=0) + timedelta(days=1) # Next day's midnight

            # Adjust sleep_start_today if it's already passed for today (e.g., if app launched after sleep time)
            # This ensures it correctly targets the sleep time for the *next* cycle (tomorrow)
            if current_time > sleep_start_today and current_time < midnight_today:
                # If current time is past sleep_start_today but before midnight, it means we are in the sleep window
                is_in_sleep_window = True
            elif current_time > sleep_start_today and current_time > midnight_today:
                # If current time is past midnight and also past sleep_start_today (meaning sleep_start_today was yesterday)
                # Then we need to check if current time is before sleep_start_today + 1 day
                # This handles the wrap-around for sleep times like 9 PM to 12 AM
                sleep_start_yesterday = sleep_start_today - timedelta(days=1)
                is_in_sleep_window = (current_time >= sleep_start_yesterday and current_time < midnight_today)
            else: # current_time < sleep_start_today
                is_in_sleep_window = False

            # Special case: If sleep time is 12 AM (00:00), it means the blocking starts at midnight.
            # The range is 9 PM to 12 AM max. So if user sets 12 AM, it's 00:00.
            # If current time is between user's set sleep hour (e.g., 9 PM) and 12 AM (00:00)
            # OR if the sleep time is 12 AM and it's past 12 AM (but before the next day's 6 AM if we had that)
            
            # The user wants blocking from 9 PM to 12 AM max.
            # So, if sleep_start_hour is 21 (9 PM) and current hour is 21, 22, 23 (9, 10, 11 PM)
            # OR if sleep_start_hour is 0 (12 AM) and current hour is 0 (12 AM)
            
            # Simplified logic for 9 PM to 12 AM max:
            # If current hour is >= sleep_start_hour AND current hour < 24 (up to midnight)
            # OR if sleep_start_hour is 0 (12 AM) and current hour is 0 (12 AM)
            
            # Let's refine based on the explicit range 9 PM to 12 AM max.
            # This means if user sets 9 PM, blocking is from 9 PM to 11:59:59 PM.
            # If user sets 10 PM, blocking is from 10 PM to 11:59:59 PM.
            # If user sets 12 AM, blocking is from 12 AM to 12 AM (effectively no blocking unless the window is extended)
            # Given "variable from 9pm to 12am max", it implies the start time.
            # The end time is always midnight (00:00 of the next day).

            # Check if current time is within the sleep blocking window
            # The window is from self.sleep_start_hour:self.sleep_start_minute until 23:59:59 (or 00:00 of next day)
            # Or if the sleep time is set to 00:00 (12 AM), then it means blocking starts right at midnight.

            # Example: Sleep time set to 21:00 (9 PM)
            # Blocking active if current_time >= 21:00 and current_time < 24:00 (next day's 00:00)

            # Example: Sleep time set to 00:00 (12 AM)
            # Blocking active if current_time >= 00:00 and current_time < 24:00 (next day's 00:00)
            # This implies blocking for the *entire* next day if current_time >= 00:00.
            # This seems to be a misunderstanding of "12am max".
            # "variable from 9pm to 12am max" implies the *start* of the sleep period.
            # The *end* of the sleep period is usually implicit (e.g., 6 AM, or just until user unblocks).
            # Given the previous context of "12 AM - 6 AM", it's likely the blocking *ends* at 6 AM.
            # But the user said "remove the 12am clock check thing i want it to go all night all day without being disturbed, i mean, it can tell me 12 is here you can go to sleep but doesn't block anything at all"
            # And then "readd the midnight and when the user launches it asks them for the time they wanna sleep."
            # And "a sleep timer that is variable from 9pm to 12am max."

            # This is a bit contradictory. Let's interpret "variable from 9pm to 12am max" as the *start* time.
            # And the blocking *ends* when the user manually dismisses it or until a fixed time like 6 AM.
            # Given the request to "readd the midnight", it implies the blocking should occur *around* midnight.

            # Let's assume the sleep blocking window is from the user-set `sleep_start_hour:sleep_start_minute`
            # until a fixed end time, say 6 AM (06:00) of the *next* day.
            # This covers the "go to sleep" aspect and allows for variable start.

            sleep_end_hour = 6 # 6 AM
            sleep_end_minute = 0 # 0 minutes

            sleep_start_dt = current_time.replace(hour=self.sleep_start_hour, minute=self.sleep_start_minute, second=0, microsecond=0)
            sleep_end_dt = current_time.replace(hour=sleep_end_hour, minute=sleep_end_minute, second=0, microsecond=0)

            # If sleep start time is in the evening (e.g., 9 PM) and end is next morning (e.g., 6 AM)
            if sleep_start_dt > sleep_end_dt: # e.g., 21:00 > 06:00
                # The sleep window spans across midnight
                is_sleep_time = (current_time >= sleep_start_dt) or \
                                (current_time < sleep_end_dt)
            else: # If sleep start time is in the morning (e.g., 00:00) and end is also morning (e.g., 06:00)
                is_sleep_time = (current_time >= sleep_start_dt and current_time < sleep_end_dt)

            if is_sleep_time:
                if not self.sleep_overlay:
                    self.root.after(0, self.create_sleep_overlay)
            else:
                if self.sleep_overlay:
                    self.root.after(0, self.remove_sleep_overlay)
            
            time.sleep(30) # Check every 30 seconds

    def create_sleep_overlay(self):
        """
        Creates a fullscreen overlay specifically for sleep time.
        This is like a gentle but firm nudge from your digital assistant,
        telling you it's time to put work aside and rest. It covers your
        screen and blocks input to help you disengage.
        """
        if self.sleep_overlay:
            return
            
        self.sleep_overlay = tk.Toplevel(self.root)
        self.sleep_overlay.attributes('-fullscreen', True, '-topmost', True)
        self.sleep_overlay.configure(bg='black')
        
        message = tk.Label(
            self.sleep_overlay,
            text="It's your designated sleep time.\nPlease get some rest.",
            font=('Arial', 28, 'bold'),
            fg='white',
            bg='black',
            justify='center',
        ...
        def update_time():
            if self.sleep_overlay:
                ...
                self.sleep_overlay.after(1000, update_time)
        
        update_time()
        self.block_input() # Block both mouse and keyboard
        self.mute_audio() # Mute audio

    def remove_sleep_overlay(self):
        """
        Removes the sleep time blocking overlay.
        This is like the sun rising and your alarm clock turning off.
        It signals the end of your enforced rest period and restores
        full access to your system.
        """
        if self.sleep_overlay:
            self.sleep_overlay.destroy()
            self.sleep_overlay = None
            self.unblock_input() # Unblock input
            self.unmute_audio() # Unmute audio
            messagebox.showinfo("Good Morning!", "Sleep time has ended. You can resume your activities.")
